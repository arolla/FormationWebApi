    ...
    public class UpdateShould : BookingControllerTests
    {
        ...
        public async Task Return_Ok_when_the_booking_exist()
        {
            ...
            this.BookingService
                ...
                .Returns(Updated.Create(new Booking(bookingId, 1, "jean@sansmail.com", ValidInput.Comment, DateTimeOffset.Now, DateTimeOffset.Now.AddDays(1), 1)));
            var response = await this.PatchAsync($"api/v1/bookings/{bookingId}", new ObjectContent<BookingPatch>(ValidInput, new JsonMediaTypeFormatter()));
