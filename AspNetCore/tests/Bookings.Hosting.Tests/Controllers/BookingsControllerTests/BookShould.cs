    ...
    public class BookShould : BookingControllerTests
    {
        ...
        public async Task Return_Created_when_a_valid_booking_is_send()
        {
            ...
            this.BookingService
                ...
                .Returns(Created.Create(new Booking(1, input.RoomId.Value, input.By, input.Comment, input.From.Value, input.To.Value, 50)));
        ...
        public async Task Return_bad_request_when_from_date_is_missing()
        {
            var input = ValidInput;
            input.From = null;
            var response = await this.HttpClient.PostAsync("api/v1/bookings", new JsonContent(input));
            Assert.AreEqual(HttpStatusCode.BadRequest, response.StatusCode);
        }
